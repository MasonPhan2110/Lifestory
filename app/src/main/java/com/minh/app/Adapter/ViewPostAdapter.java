...
public class ViewPostAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ViewPostAdapter (Home home,Context mcontext, List<Comment> mcomment){
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof PostViewHolder){
            final PostViewHolder itemholder = (PostViewHolder) holder;
            ...
            final CommentViewHolder itemholder = (CommentViewHolder) holder;
            final Comment comment;
            comment = mcomment.get(position-1);
            itemholder.username.setText(comment.getUsername());
            itemholder.comment_txt.setText(comment.getContent());
            if(comment.getProfile_img().equals("default")){
                itemholder.profile_img.setImageResource(R.mipmap.ic_launcher);
            }else {
                Glide.with(mcontext).load(comment.getProfile_img()).into(itemholder.profile_img);
            }
        }
    }
